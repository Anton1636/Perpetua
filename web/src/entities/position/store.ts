...
import { assertInvariants } from "@/shared/lib/invariants";
...
export const usePositionStore = create<PositionState>((rawSet, get) => {
  // wrap set so invariants are checked (in dev) after every state change
  const set: typeof rawSet = (partial, replace) => {
    rawSet(partial as never, replace as never);
    const s = get();
    assertInvariants(s.positions, s.available);
  };
